...
export async function onRequest(context) {
  ...
  try {
    // Build the simpler.grants.gov API URL
    const targetUrl = new URL('https://simpler.grants.gov/api/v1/opportunities');
    ...
    // Default to posted+forecasted if no status specified
    ...
    if (!response.ok) {
      ...
      console.error('simpler.grants.gov error ' + response.status + ': ' + errText);
